...
<body>
  ...
  <main class="container mx-auto px-8 py-20 flex-grow">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 
                border-b-2 border-white/40 pb-10">
      
      <div class="relative w-full md:w-[450px] blur-1">
        <input 
  ...
  <script>
    ...
    function renderGames(games) {
      ...
      const html = games.map(game => `
        <div class="group bg-[#0a0a0a] border-2 border-[#ffffff12] transition-all duration-300 flex flex-col h-full hover:shadow-none">
          <div class="relative h-56 overflow-hidden border-b-2 border-[#ffffff12]">
          ...
          <div class="p-6 flex flex-col flex-grow">
            <h4 class="text-xl font-black leading-tight mb-4 line-clamp-2">${game.name}</h4>
            <div class="mt-auto flex justify-between items-center pt-4 border-t border-[#ffffff12] text-[10px] font-bold uppercase tracking-widest">
              <span>${game.released?.split('-')[0] || 'TBA'}</span>
              <span class=" opacity-60">ID: ${game.id}</span>
            </div>
          </div>
        </div>
      `).join('');
